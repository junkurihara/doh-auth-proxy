...
pub fn is_query(packet_buf: &[u8]) -> Result<Message, Error> {
  is(packet_buf, trust_dns_proto::op::MessageType::Query)
}

pub fn is_response(packet_buf: &[u8]) -> Result<Message, Error> {
  is(packet_buf, trust_dns_proto::op::MessageType::Response)
}

fn is(packet_buf: &[u8], mtype: trust_dns_proto::op::MessageType) -> Result<Message, Error> {
...
pub fn decode(packet_buf: &[u8]) -> Result<Message, Error> {
